...
class SoforpController extends Controller
{
	...
	public function initButtons($data)
	{
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['save_and_close'] = $this->url->link("extension/extension", 'module_id=' . $this->request->get['module_id'] . '&user_token=' . $this->session->data['user_token'] . "&close=1", 'SSL');
		}

		$data['recheck'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'], 'SSL');
		$data['close'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], 'SSL');
	...
	public function install()
	{
		...
		$this->{"model_extension_" . $this->_route . "_" . str_replace('/', '_', $this->_moduleSysName())}->install();
	}

	public function upgrade()
	{
		$this->load->model("extension/". $this->_route . "/" . $this->_moduleSysName());
		$this->{"model_extension_" . $this->_route . "_" . str_replace('/', '_', $this->_moduleSysName())}->upgrade();
	}

	public function uninstall()
	{
		$this->load->model("extension/". $this->_route . "/" . $this->_moduleSysName());
		$this->{"model_extension_" . $this->_route . "_" . str_replace('/', '_', $this->_moduleSysName())}->uninstall();
	}
